...
#include "MPI_OpenMP_CG.hpp"

namespace cgcore{
    
    void MPI_OpenMP_CG::run(const double * A , const double * b, double * x, size_t size, int max_iter, double res_error) const{
        conjugate_gradient(A, b, x, size, max_iter, res_error);
    }

    double MPI_OpenMP_CG::dot(const double * x, const double * y, size_t size) const
    {
        double result = 0.0;
    ...
    void MPI_OpenMP_CG::axpby(double alpha, const double * x, double beta, double * y, size_t size) const
    {
        // y = alpha * x + beta * y
    ...
    void MPI_OpenMP_CG::gemv(double alpha, const double * A, const double * x, double beta, double * y, size_t num_rows, size_t num_cols, int num_processes, int my_rank, int * displacements, int * counts) const
    {
        // y = alpha * A * x + beta * y;
     ...
     * 
    */ 
    void MPI_OpenMP_CG::conjugate_gradient(const double * A, const double * b, double * x, size_t size, int max_iters, double rel_error) const
    {
