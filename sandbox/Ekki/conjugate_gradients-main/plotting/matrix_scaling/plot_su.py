...
def plot_data(file_path, title):
    ...
    # plot fit func
    plt.plot(size_fit, parabula(size_fit, *popt2), linewidth=0.9, color='tab:green', label=rf'Fit func parallel: $a \cdot x^2$, $a = ${np.format_float_scientific(popt2[0], precision=3, exp_digits=1)}')
    plt.plot(size_fit, parabula(size_fit, *popt3), linewidth=0.9, color='tab:red', label=rf'Fit func sequential: $a \cdot x^2$, $a = ${np.format_float_scientific(popt3[0], precision=3, exp_digits=1)}')
